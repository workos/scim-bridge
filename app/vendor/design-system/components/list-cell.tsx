...
import { type FlexProps, Flex } from './flex.js';
...
const ListCellRoot = React.forwardRef<HTMLDivElement, ListCellRootProps>(
  function ListCellRoot({ children, size = '1', ...props }, forwardedRef) {
...
interface ListCellItemProps extends Omit<
...
const ListCellItem = React.forwardRef<HTMLDivElement, ListCellItemProps>(
  function ListCellItem(
    ...
    const handleNodeKeyDown = React.useCallback((event: KeyboardEvent) => {
      ...
      if (event.key === 'ArrowUp') {
        ...
        focusIfPossible(
          itemRef.current.previousElementSibling || itemRef.current,
        );
        ...
        if (
          focusIfPossible(
            tabbableNodes.current[currentNodeIndex - 1]?.element ||
              itemRef.current,
          )
        ) {
          event.preventDefault();
      ...
      if (event.key === 'Tab' && itemRef.current) {
        const nextElementToFocus =
          tabbableNodes.current[
            event.shiftKey ? currentNodeIndex - 1 : currentNodeIndex + 1
          ]?.element;
        ...
        <Text asChild size="2">
          <Component
            ...
            onKeyDown={(event) => {
              ...
              if (
                !tabbableNodes.current ||
                tabbableNodes.current.length === 0
              ) {
                return;
              }
...
const ListCellItemSlot = React.forwardRef<
  HTMLDivElement,
  ListCellItemSlotProps
>(function ListCellItemSlot({ side = 'left', ...props }, forwardedRef) {
  return (
    <div
      ref={forwardedRef}
      className={classNames('ListCellItemSlot', side)}
      {...props}
    />
  );
});
...
export {
  ListCellItem as Item,
  ListCellItemSlot as ItemSlot,
  ListCellRoot as Root,
};
