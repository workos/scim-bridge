...
const useAccordionContext = () => {
  ...
  if (!accordionContext) {
    throw new TypeError(
      '`useAccordionContext` must be called within `AccordionContext.Provider`',
    );
  }
...
interface AccordionHeaderProps
  extends React.ComponentPropsWithRef<'h3'>, AccordionHeaderOwnProps {}
...
>(({ as = 'h3', className, children, ...props }, forwardedRef) => {
  ...
  return (
    <AccordionPrimitive.Header asChild>
      <Tag
        ref={forwardedRef}
        className={classNames('AccordionHeader', className)}
        {...props}
      >
...
>(
  (
    { children, className, hiddenUntilFound = true, ...props },
    forwardedRef,
  ) => {
    const context = useAccordionContext();
    const itemContext = useAccordionItemContext();
    const contentInnerRef = React.useRef<HTMLDivElement>(null);
    const [contentInnerHeight, setContentInnerHeight] = React.useState<
      number | null
    >(null);
