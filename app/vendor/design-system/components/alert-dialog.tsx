...
import * as ThemesAlertDialog from '../radix-themes/components/alert-dialog.js';
...
type AlertDialogTitleProps = React.ComponentPropsWithoutRef<
  typeof ThemesAlertDialog.Title
>;
...
>((props, forwardedRef) => (
  <ThemesAlertDialog.Title ref={forwardedRef} size="4" {...props} />
));
  ...
  >
    ...
    {(() => {
      ...
      if (typeof description === 'string' || isReactFragment(description)) {
        return (
          <AlertDialogDescription mt="-3">{description}</AlertDialogDescription>
        );
      }
