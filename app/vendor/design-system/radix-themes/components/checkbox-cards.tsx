...
const [createCheckboxCardsContext] = Context.createContextScope(
...
const [CheckboxCardsProvider, useCheckboxCardsContext] =
  createCheckboxCardsContext<CheckboxCardsContextValue>(
    CHECKBOX_CARD_GROUP_NAME,
  );

type CheckboxCardsRootElement = React.ElementRef<
  typeof CheckboxGroupPrimitive.Root
>;
type CheckboxCardsRootOwnProps = GetPropDefTypes<
  typeof checkboxCardsRootPropDefs
>;
...
interface CheckboxCardsItemProps
  extends
    ComponentPropsWithout<typeof CheckboxGroupPrimitive.Item, RemovedProps>,
    MarginProps {}
...
>(
  (
    { __scopeCheckboxCards, children, className, style, ...props },
    forwardedRef,
  ) => {
    ...
    return (
      <label
        className={classNames('rt-BaseCard', 'rt-CheckboxCardsItem', className)}
        style={style}
      >
        {children}
...
export { CheckboxCardsItem as Item, CheckboxCardsRoot as Root };
export type {
  CheckboxCardsItemProps as ItemProps,
  CheckboxCardsRootProps as RootProps,
};
