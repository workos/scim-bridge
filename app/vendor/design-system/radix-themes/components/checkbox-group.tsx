...
const [createCheckboxGroupContext] = Context.createContextScope(
  CHECKBOX_GROUP_NAME,
  [createCheckboxGroupScope],
);
const useCheckboxGroupScope = createCheckboxGroupScope();

type CheckboxGroupRootOwnProps = GetPropDefTypes<
  typeof checkboxGroupRootPropDefs
>;
...
>((_props: ScopedProps<CheckboxGroupItemProps>, forwardedRef) => {
  ...
  const { size } = useCheckboxGroupContext(
    'CheckboxGroupItem',
  ...
  if (children) {
    return (
      ...
      >
        ...
        />
        {children && (
          <span className="rt-CheckboxGroupItemInner">{children}</span>
        )}
      </Text>
    );
  }
...
export { CheckboxGroupItem as Item, CheckboxGroupRoot as Root };
export type {
