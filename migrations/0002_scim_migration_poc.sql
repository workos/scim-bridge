...
-- The bundled endpoints' bearer tokens (native.scim_token, mock_workos.scim_token)
-- are NOT seeded here: a secret whose value depends on the datastore engine is a
-- bad property, so the app mints them at boot (server/config.ts, shared/ids.ts).
...
CREATE TABLE IF NOT EXISTS scim_directories (
  -- Ids and tokens are generated by the app (workers/shared/ids.ts), not by a
  -- column default, so every datastore driver produces the same shapes.
  id TEXT PRIMARY KEY,
  name TEXT NOT NULL,
